...
<p align="center">
  <img src="assets/fact-check-x-overview.png?v=1.1.15" alt="Fact-Check-X 多平台事实核验：完整采集、知识点对比、权威核验与答案生成、平台表现评估" width="900">
</p>
...
2. **各方答案聚合（未核验）**：原子事实、各家主张、共识、冲突与引用忠实性；直接答案和补充参考在概览与逐知识点明细中分别呈现。
3. **权威核验后的最终答案**：只展示有证据支持的直接答案、权威来源和证据边界，不在本阶段计算平台评分。
4. **各方答案测评报告**：只读取第三步锁定结果，在一个逐知识点核验明细区内分开呈现直接答案与补充参考，再展示准确性、完整性、来源质量和平台差异；官方无法查证的主张统一标为“疑似误导”并移入补充参考风险区，长依据默认收起，不重新计算权威结论。

每一步都必须先向用户展示真实可打开的产物，再等待用户明确选择继续、修正或结束。程序用一次性令牌和产物摘要执行硬门禁；用户最初要求“完整跑完”也不能替代逐阶段确认，不存在绕过确认的自动推进模式。
...
- 无证据时在结构化数据中保留 `insufficient_evidence` 供审计，面向用户统一显示为“疑似误导”，移入补充参考风险区，不写入确定答案和准确率分母。
- 可信搜索、来源采集等技术动作失败时原则上停止；若流程仍继续，相关主张仍按“疑似误导”呈现，并在每一步报告顶部显著提示本次技术失败及其影响。
...
第四步只负责平台裁决、指标和表现比较，并承接第三步锁定的逐知识点核验明细。直接答案与补充参考分区展示，不重写第三步报告或权威结论。覆盖率只衡量平台是否回答了合法直接知识点；答到即计覆盖，证据是否充分由证据充分率单独体现。仅当某知识点在所有平台中都是未回答或证据不足时，才整体移出覆盖率与准确率分母。

“编造式幻觉”和“证据不足”只作为内部原始裁决状态保留；面向用户统一显示为“疑似误导：官方无法查证，有可能过期、编造或信息源误导”。这类主张移入补充参考风险区，并在报告上方突出显示；每个知识点的长依据默认收起，点击后可查看完整原文。
...
1. 在同一登录会话提交原问题。
2. 等待普通回答停止生成。
3. 点击回答下方的“深度溯源”入口，不重复提交问题。
4. 识别同页弹出的结果；若产品打开同源结果页，则接管该页面。
5. 等待深度溯源结果停止生成。
6. 独立保存回答、来源、截图和页面存证。

按钮缺失、结果未出现或结果未完成均为采集失败。两个结果同时选择时，采集器只提交一次原问题，但会分别保存回答、引用、截图和 HTML。它在 `results.json` 和报告中是一个独立平台结果。普通深知晓与深度溯源都只能凭本次回答自身所附、与当前主张可定位且语义一致的官方材料独立形成锚点；深度溯源不能继承普通深知晓的结果。
...
该命令会先自检，首次缺失时根据包内锁文件自动安装依赖并验证采集 CLI；依赖已完整时直接跳过。它不会借用其他技能或旧安装目录中的 `node_modules`，也不会下载 Playwright 测试浏览器。
...
- `no_evidence` 在结构化数据中归入 `insufficient_evidence`，不冒充已经证实的错误；面向用户统一显示为“疑似误导”，移入补充参考风险区，并继续生成第四步报告。
...
| 可信搜索超时/断网 | 服务状态与网络 | 保留现有 Key 并自动重试三次；仍失败原则上按技术错误停止，不清除凭据；若流程继续，则每步显著提示技术失败，相关主张按“疑似误导”展示 |
| `insufficient_evidence` | 当前检索未取得足够证据 | 内部保留原始状态，面向用户显示“疑似误导”并移入补充参考风险区；不写入确定答案或准确率分母，继续生成第四步报告 |
