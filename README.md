...
<p align="center">
  <img src="assets/fact-check-x-overview.png?v=1.1.12" alt="Fact-Check-X 多平台事实核验：完整采集、知识点对比、权威核验与答案生成、平台表现评估" width="900">
</p>
...
4. **各方答案测评报告**：只读取第三步锁定结果，在一个逐知识点核验明细区内分开呈现直接答案与补充参考，再展示准确性、完整性、来源质量和平台差异；不重新计算权威结论。
...
- `no_evidence` 归入 `insufficient_evidence`，不自动等于编造，也不阻断第四步报告。
...
| 可信搜索超时/断网 | 服务状态与网络 | 保留现有 Key 并自动重试三次；仍失败则按技术错误停止，不清除凭据 |
| `insufficient_evidence` | 当前检索未取得足够证据 | 明确保留证据边界，不写入确定答案或准确率分母，继续生成第四步报告 |
