...
description: 对单个知识点调用可信搜索取得官方证据，由当前运行载体裁决各平台主张，并分别生成权威核验后的最终答案与各方答案测评报告。支持多知识点独立并发、深知晓官方材料锚点免重复检索，以及直接准确、间接准确、结果巧合、严重误导、凭空编造等分类。可用于 Fact-Check-X 完整流程，也可作为独立的权威证据核验能力使用。
license: Apache-2.0
---
...
该报告逐知识点展示权威结论、权威证据、核验方式和证据边界，不展示平台评分或平台裁决表。平台集合必须与 `verification.json` 完全一致，缺少任一已选平台裁决时拒绝生成；平台数量由输入决定，只要求 `N≥1`。报告顶部必须先展示与全部知识点 ID 完整绑定的 `finalAnswer`，作为本阶段可直接使用的权威核验后最终答案；证据不足知识点必须从确定答案中排除并单列。
...
第四步报告必须保留平台表现、逐知识点裁决、原始回答与引用存证，并通过链接引用第三步的权威证据，不重复计算或改写第三步结论；按本次实际平台集合动态布局。
