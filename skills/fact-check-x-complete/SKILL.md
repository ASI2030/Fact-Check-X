...
description: 面向真实业务决策的多平台事实核验 Skill。正式支持深知晓、深知晓（深度溯源）、豆包、腾讯元宝、DeepSeek 和通义千问，按用户输入动态选择 N≥1 个平台。完整采集原回答、引用、截图与页面存证，结构化比较关键事实，按需使用可信搜索逐点核验权威证据，并评估各平台表现。权威答案是核验产物之一，原始证据、冲突与证据边界全程可追溯。基础采集与对比无需 API Key；可信搜索是权威核验阶段的可选增强。语义工作由 Codex、Claude Code、WorkBuddy 等当前运行载体完成，不调用外部大模型 API。
license: Apache-2.0
metadata:
  slug: fact-check-x
...
![Fact-Check-X 多平台事实核验：完整采集、知识点对比、权威核验与答案生成、平台表现评估](https://raw.githubusercontent.com/ASI2030/Fact-Check-X/main/assets/fact-check-x-overview.png?v=1.1.16)
...
- 第四步覆盖率只衡量“答没答到”：答到即算覆盖，证据是否充分由证据充分率单独体现。仅当一个知识点在所有平台中都是未回答或证据不足时，才整体移出覆盖率与准确率分母。
- “编造式幻觉”和“证据不足”只作为内部原始裁决状态保留；对用户统一显示为“疑似误导”，移入补充参考风险区并在报告上方突出。每个知识点的长依据默认收起，点击后可看完整原文。
- 来源采集、可信搜索等技术动作失败时原则上停止；若流程仍继续，每一步报告都必须在显著位置提示技术失败及影响，相关主张继续按“疑似误导”呈现。
...
旧安装残留的 `node_modules`，也不要求用户手工进入目录安装依赖。
...
上述命令固定使用交互模式。每次向用户展示当前阶段产物并收到继续确认后执行：
...
`修正当前结果` 使用 `--decision revise`，`到此结束并保留产物` 使用 `--decision stop`；两者都不允许进入下一阶段。
...
`deliver` 只能读取第三步已锁定的 `verification.json` 和 `03-authority-report.html`；摘要不一致时必须拒绝，不得重算、覆盖或变更第三步结论。第四步承接逐知识点核验明细，直接答案与补充参考分区展示；旧的重复裁决矩阵不得再次生成。技术流程成功但存在 `evidenceGaps` 时继续交付第四步：底层原始状态继续保留，用户界面统一显示为“疑似误导”，移入补充参考风险区，不写入确定答案或准确率分母。

可信搜索返回成功但当前知识点 `no_evidence` 时，底层归类为 `insufficient_evidence`。单次检索未返回材料不等于已经证实平台主张错误；程序必须保留原始证据边界，面向用户标为“疑似误导：官方无法查证，有可能过期、编造或信息源误导”，移入补充参考风险区并继续后续报告。覆盖率只衡量是否答到；只有该知识点在所有平台中都未回答或证据不足时，才整体退出覆盖率和准确率分母。
...
- `04-final-report.html`：按本次动态平台集合生成的各方答案测评报告；
...
- 任何登录失败、验证码、搜索错误、证据不足和抽取异常都必须显式报告，不得伪装成功。技术动作失败后若流程仍继续，每一步报告都必须在显著位置统一提示，相关无法核验的主张按“疑似误导”呈现。
- 深知晓与深知晓（深度溯源）的可信搜索材料统一按官方材料处理。两者都只能凭本次回答自身所附、与主张可定位且语义一致的官方材料独立生成 `trustedAnchor.eligible=true`；深度溯源不继承普通深知晓的结果。
