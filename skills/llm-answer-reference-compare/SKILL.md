...
description: 用 Playwright 从深知晓、深知晓（深度溯源）、豆包、腾讯元宝、DeepSeek 和通义千问网页端无损采集完整回答、原始引用、引用标记和现场存证。用于 Fact-Check-X 的原始答案采集阶段，或任何需要比较多个 AI 网页回答与参考来源的任务。
license: Apache-2.0
---
...
采集一个或更多平台。提交前的页面关闭等确定性故障可自动重试；问题已经尝试提交、确认提交或页面已经出现回答后，禁止重新提问，只能在原会话继续等待或恢复提取：
